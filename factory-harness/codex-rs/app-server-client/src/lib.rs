...
pub use codex_app_server::AppServerExtensionInstaller;
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        Self::start_with_extension_installers(args, Vec::new()).await
    }

    /// Starts the in-process runtime with additional native Codex extension
    /// contributors installed after the built-in contributors.
    pub async fn start_with_extension_installers(
        args: InProcessClientStartArgs,
        extension_installers: Vec<AppServerExtensionInstaller>,
    ) -> IoResult<Self> {
        Self::start_with_extension_installers_and_options(
            args,
    ...
    /// Starts the in-process runtime with additional native Codex extension
    /// contributors and explicit startup-task behavior.
    pub async fn start_with_extension_installers_and_options(
        args: InProcessClientStartArgs,
        extension_installers: Vec<AppServerExtensionInstaller>,
        plugin_startup_tasks: codex_app_server::PluginStartupTasks,
    ) -> IoResult<Self> {
        let channel_capacity = args.channel_capacity.max(1);
        let mut handle = codex_app_server::in_process::start_with_extension_installers_and_options(
