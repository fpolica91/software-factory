...
impl Session {
    ...
    /// Runs the generic extension post-processing chain before emitting one
    /// host-created lifecycle item. Parsed model items use their existing
    /// stream-finalization path instead, so every item is contributed once.
    pub(crate) async fn emit_contributed_turn_item_started(
        &self,
        turn_context: &TurnContext,
        item: &TurnItem,
    ) {
    ...
    /// Runs the generic extension post-processing chain before emitting one
    /// host-created completed item. See `emit_contributed_turn_item_started`.
    pub(crate) async fn emit_contributed_turn_item_completed(
        &self,
        turn_context: &TurnContext,
        mut item: TurnItem,
    ) {
