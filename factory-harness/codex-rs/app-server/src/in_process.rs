...
use crate::AppServerExtensionInstaller;
...
pub async fn start(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    start_with_extension_installers(args, Vec::new()).await
}

/// Starts an in-process app-server runtime with additional native Codex
...
pub async fn start_with_extension_installers(
    ...
    start_with_extension_installers_and_options(
        args,
...
/// Starts an in-process app-server runtime with additional native Codex
/// extension contributors and explicit startup-task behavior.
pub async fn start_with_extension_installers_and_options(
    mut args: InProcessStartArgs,
    extension_installers: Vec<AppServerExtensionInstaller>,
    plugin_startup_tasks: crate::PluginStartupTasks,
) -> IoResult<InProcessClientHandle> {
    ...
    let client = start_uninitialized(args, extension_installers, plugin_startup_tasks).await?;
